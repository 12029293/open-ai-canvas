        ...
        <div className="flex flex-col gap-6">
            <section className="settings-preference-block">
                <div className="settings-preference-heading">
                    ...
                    <p>支持 HTTP / HTTPS / SOCKS5；用户名含 <code>&#123;sid&#125;</code> 的动态代理每次提交任务自动轮换出口 IP。删除代理会自动解绑所有账号（回到直连）。</p>
                </div>
                        ...
                        { title: "名称", dataIndex: "name", width: 180 },
                        {
                            ...
                            render: (_: unknown, proxy: NetworkProxyView) => (
                                <span className="flex items-center gap-1.5">
                                    ...
                                    {(proxy.username ?? "").includes("{sid}") ? (
                                        <span className="rounded bg-primary/10 px-1.5 py-0.5 text-xs text-primary">动态轮换</span>
                                    ) : null}
                                </span>
                            ),
                        },
            ...
            >
                <div className="flex flex-col gap-3 pt-2">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <label className="flex flex-col gap-1 text-sm">
                            用户名（可选）
                            <Input value={form.username} autoComplete="off" placeholder="如 user-sid-&#123;sid&#125;" onChange={(event) => setForm({ ...form, username: event.target.value })} />
                            <span className="text-xs text-muted-foreground">
                                动态住宅代理（NexIP 等）可在用户名中填 <code>&#123;sid&#125;</code> 占位符：每次提交任务自动换一个出口 IP，任务期间保持同一 IP；不填则始终使用固定 IP。
                            </span>
                        </label>
                        <label className="flex flex-col gap-1 text-sm">
                            密码（可选）
