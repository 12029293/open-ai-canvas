...
// maxProxyTransports Transport 缓存上限：动态轮换代理每次取号渲染新的会话
// 标识（新地址），超限时清退其他条目（在用请求不受影响，空闲连接随后关闭）。
const maxProxyTransports = 64

func evictProxyTransports(keepURL string) {
	if len(proxyTransports) < maxProxyTransports {
		return
	}
	for url, transport := range proxyTransports {
		if url != keepURL {
			delete(proxyTransports, url)
