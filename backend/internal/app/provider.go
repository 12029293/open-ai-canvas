...
func (s *Service) processCanvasGenerationTask(ctx context.Context, userID string, taskProjectID string, taskType string, fallbackPrompt string, rawInput string) (map[string]interface{}, error) {
	...
	// 网页中继渠道凭据来自账号池服务，任务配置不携带渠道密钥，只要求模型名。
	if !isWebRelayInterface(input.Config.InterfaceType) && (strings.TrimSpace(input.Config.BaseURL) == "" || strings.TrimSpace(input.Config.APIKey) == "" || strings.TrimSpace(input.Config.Model) == "") {
