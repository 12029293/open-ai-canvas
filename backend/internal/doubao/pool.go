...
func (s *Service) PickSite(site string, preferID string) (*ActiveCredential, error) {
	...
	if chosen == nil {
		for i := range accounts {
			if accounts[i].ID == active && s.usable(&accounts[i], now) && quotaOK(&accounts[i]) {
				chosen = &accounts[i]
				break
			}
		}
	...
	return &ActiveCredential{ID: chosen.ID, Label: chosen.Label, Site: chosen.Site, SessionID: chosen.SessionID, CookieHeader: chosen.CookieHeader, ProxyURL: chosen.ProxyURL}, nil
}
