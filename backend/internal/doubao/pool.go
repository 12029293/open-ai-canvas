...
//
// 账号池语义移植自豆包创作工作台 src/account-store.js：
//   - 文生图 / 文生视频必须从账号池取号；
...
const (
	...
	// FailKindNetworkUnreachable 出站链路失败（代理拒绝/断网/拨号超时）。
	// 出站代理绑定在账号上：这类失败换号重试有意义，属于账号类失败。
	FailKindNetworkUnreachable = "network_unreachable"
)
...
// ProxyURL 为该账号绑定的出站代理（空 = 直连），生成链路经 WithProxy 注入 context。
...
func NewService(db *gorm.DB) *Service {
	return &Service{db: db, busy: make(map[string]int)}
}

// ReleaseAccount 释放账号占用（生成结束/失败/换号后必须调用；幂等）。
func (s *Service) ReleaseAccount(id string) {
	if id == "" {
		return
	}
	s.mu.Lock()
...
// ErrAllAccountsBusy 站点内可用账号都在生成中。生成链路据此等待空闲账号
// （而不是立即失败），让超出账号数的任务自然排队轮转。
var ErrAllAccountsBusy = errors.New("站点内所有可用账号都在生成中，请稍后再试或补充账号")

// dolaDailyVideoQuota Dola 站点单账号每日视频生成条数上限（跨天自动恢复）。
const dolaDailyVideoQuota = 2

const quotaDateFormat = "2006-01-02"

// dolaQuotaUsed 返回账号当日已用视频配额：归属日不是今天则视为 0。
func dolaQuotaUsed(a *model.DoubaoAccount, today string) int {
	if a.VideoQuotaDate != today {
		return 0
	}
	return a.VideoCountUsed
}

// UsableCount 统计站点内当前可领号的账号数（启用、登录有效、不在冷却、配额未满）。
// 账号池渠道用它动态放开渠道并发上限：有多少可用账号就允许多少任务并行。
func (s *Service) UsableCount(site string) int {
	site = NormalizeSite(site)
	s.mu.Lock()
	defer s.mu.Unlock()
	accounts, _, err := s.loadAccounts(site)
	if err != nil {
		return 0
	}
	now := time.Now()
	today := now.Format(quotaDateFormat)
	count := 0
	for i := range accounts {
		if s.usable(&accounts[i], now) &&
			(site != SiteDola || dolaQuotaUsed(&accounts[i], today) < dolaDailyVideoQuota) {
			count++
		}
	}
	return count
...
// PickSite 取指定站点的可用账号：优先指定 prefer，其次按最近使用时间轮转
// （最久未用的可用账号先领号），实现多账号依次循环、并行分摊生成任务；
// 正在生成中的账号（busy）本轮跳过，一个账号同一时间只领一个任务。
// Dola 站点受每日视频配额约束：当日已满的账号本轮跳过（豆包/即梦不受限）。
// 成功取号会记录 useCount/lastUsedAt、设为该站点的活跃账号并标记占用，
// 生成结束后调用 ReleaseAccount 释放。
func (s *Service) PickSite(site string, preferID string) (*ActiveCredential, error) {
	...
	now := time.Now()
	quotaApplies := site == SiteDola
	today := now.Format(quotaDateFormat)
	quotaOK := func(a *model.DoubaoAccount) bool {
		return !quotaApplies || dolaQuotaUsed(a, today) < dolaDailyVideoQuota
	}
	free := func(a *model.DoubaoAccount) bool {
		return s.busy[a.ID] == 0
	}
	var chosen *model.DoubaoAccount
	if preferID != "" {
		for i := range accounts {
			if accounts[i].ID == preferID && s.usable(&accounts[i], now) && quotaOK(&accounts[i]) {
				if !free(&accounts[i]) {
					return nil, errors.New("指定账号正在生成中，请等待当前任务完成")
				}
				chosen = &accounts[i]
				break
			}
	...
	if chosen == nil {
		// LRU 轮转：最久未使用（从未用过的最优先）的可用账号先领号；
		// 同为从未使用时保持列表顺序，保证取号结果稳定可预期。
		older := func(a, b *model.DoubaoAccount) bool {
			if a.LastUsedAt == nil {
		...
		for i := range accounts {
			a := &accounts[i]
			if !s.usable(a, now) || !quotaOK(a) || !free(a) {
				continue
			}
			if chosen == nil || older(a, chosen) {
				chosen = a
			}
		}
	}
	if chosen == nil {
		// 先区分「有号但全忙」与「真的无号」：全忙返回可识别的忙错误，
		// 生成链路会等待空闲账号；真无号再按配额/可用性给出原始错误。
		for i := range accounts {
			if s.usable(&accounts[i], now) && quotaOK(&accounts[i]) {
				return nil, ErrAllAccountsBusy
			}
		}
		if quotaApplies {
			return nil, errors.New("Dola 账号今日视频条数已用完（每日 2 条，跨天自动恢复）")
		}
	...
	// 标记占用：生成结束后由调用方 ReleaseAccount 释放。
	s.busy[chosen.ID]++
	// 代理在取号时实时解析：动态轮换代理（用户名含 {sid}）按账号 ID 派生固定
	// 会话标识，同一账号始终复用同一出口 IP（IP 轨迹稳定，降低风控概率），
	// 不同账号之间自然分散到不同出口；解析失败时回退绑定时的冗余快照。
	proxyURL := chosen.ProxyURL
	if chosen.ProxyID != "" {
		if resolved := netproxy.LookupURLStable(s.db, chosen.ProxyID, chosen.ID); resolved != "" {
			proxyURL = resolved
		}
	}
...
func (s *Service) NoteDolaVideoSuccess(id string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		used := dolaQuotaUsed(&a, today) + 1
		return tx.Model(&model.DoubaoAccount{}).Where("id = ?", id).
			Updates(map[string]any{"video_count_used": used, "video_quota_date": today}).Error
	})
}

// AssignProxy 给一批账号绑定/解绑出站代理（proxyURL 为空 = 恢复直连）。
// 代理被删除时由 app 层先调用本方法解绑，再删除代理记录。
func (s *Service) AssignProxy(ids []string, proxyID, proxyURL string) (int64, error) {
	if len(ids) == 0 {
		return 0, errors.New("未选择任何账号")
	}
	s.mu.Lock()
	...
	res := s.db.Model(&model.DoubaoAccount{}).
		Where("id IN ?", ids).
		Updates(map[string]any{"proxy_id": proxyID, "proxy_url": proxyURL, "updated_at": time.Now()})
	return res.RowsAffected, res.Error
}
