...
type ClassifyError struct {
	...
	Code    int64
	Message string
	// CooldownUntil 周期性额度耗尽时由上游声明恢复时间，冷却到该时刻
	//（零值 = 按 Kind 默认冷却策略）。
...
var (
	quotaPatterns = regexp.MustCompile(`免费次数已用完|额度已用完|额度用完|额度用尽|次数已用完|开通豆包专业版|积分不足|余额不足|今日额度|已达上限`)
	// weeklyQuotaPatterns 周期性额度耗尽的会话回复（"近 7 天的额度用完了，
	// 预计9月28日 17:00 恢复，升级你的订阅套餐…"）。这是任务终态：上游不会
	// 再推进，白等只会浪费 20 分钟轮询窗口。只对助手回复文本判定，不扫原始
	// 页面——页面 UI 里的套餐促销文案会造成误伤。
	weeklyQuotaPatterns = regexp.MustCompile(`额度用完了|恢复为你服务|升级你的订阅套餐|加购创作额度包`)
	weeklyQuotaRecovery = regexp.MustCompile(`预计\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日\s*(\d{1,2})[:：](\d{2})`)
...
// proxyCtxKey 在 context 里携带账号绑定的出站代理地址（WithProxy / proxyFromCtx）。
type proxyCtxKey struct{}

// WithProxy 把代理地址写入 context。空串同样写入（覆盖旧值），
// 供生成循环换号时清除上一轮账号的代理；clientForProxy 视空串为直连。
func WithProxy(ctx context.Context, proxyURL string) context.Context {
	return context.WithValue(ctx, proxyCtxKey{}, strings.TrimSpace(proxyURL))
}

func proxyFromCtx(ctx context.Context) string {
	if ctx != nil {
		if p, ok := ctx.Value(proxyCtxKey{}).(string); ok {
...
// proxiedClients 按代理地址缓存 http.Client（socks5 与 http/https 代理各自的 Transport）。
var proxiedClients sync.Map // proxyURL -> *http.Client

// maxProxiedClients 缓存上限：动态轮换代理每次取号渲染新的会话标识，
// 地址集合无界增长；超限时清退其他条目（在用客户端仍持有引用，
// 旧 Transport 的空闲连接超时后自动关闭，不会泄漏）。
const maxProxiedClients = 64

func evictProxiedClients(keepURL string) {
	count := 0
	proxiedClients.Range(func(_, _ any) bool {
		count++
		return true
	})
	if count < maxProxiedClients {
		return
	}
	proxiedClients.Range(func(key, value any) bool {
		if url, _ := key.(string); url != keepURL {
			proxiedClients.Delete(key)
...
// clientForProxy 返回该 ctx 对应出站链路的 HTTP 客户端：
// 无代理时复用共享的直连客户端；有代理时按地址缓存独立客户端。
// 代理构建失败（非法地址）时回退直连，不阻断生成。
func clientForProxy(ctx context.Context) *http.Client {
	proxyURL := proxyFromCtx(ctx)
	if proxyURL == "" {
		return httpClient
	}
	if cached, ok := proxiedClients.Load(proxyURL); ok {
	...
	if err != nil || parsed.Scheme == "" || parsed.Host == "" {
		log.Printf("[doubao] 代理地址无效，回退直连：%.120s", proxyURL)
		return httpClient
	}
...
func samanthaPost(ctx context.Context, cookieHeader, apiPath string, body any, timeout time.Duration, tabID string, accept string) (string, error) {
	...
	res, err := clientForProxy(ctx).Do(req)
	if err != nil {
		// 任务级取消：原样返回，不能当成账号类失败去换号。
		if ctx.Err() != nil {
			return "", err
		}
		// 传输层失败（代理拒绝/断网/TLS/请求超时）出在所选账号绑定的出站链路上：
		// 归类为账号类失败驱动换号重试，而不是把整个任务判死。
		// 之前裸返回 url.Error，Generate*WithPool 视为「换号无意义」直接失败，
		// 一个账号的代理挂掉就会连累整个任务。
		return "", &ClassifyError{Kind: FailKindNetworkUnreachable, Message: describeTransportError(err)}
	}
...
// 传输层错误里常见的出站端点形态：SOCKS 代理拨号与 HTTP 代理CONNECT。
var (
	socksProxyEndpointPattern   = regexp.MustCompile(`socks connect tcp ([^\s>]+)`)
	httpProxyEndpointPattern    = regexp.MustCompile(`proxyconnect tcp: dial tcp ([^\s>]+)`)
	directDialEndpointPattern   = regexp.MustCompile(`dial tcp ([^\s>]+?)\s*->`)
	directTargetEndpointPattern = regexp.MustCompile(`dial tcp ([^\s>]+)`)
)

// describeTransportError 把 Go 传输层错误压成给用户看的中文摘要，
// 只保留出站端点和根因，丢弃完整上游 URL（内含设备指纹，且会让前端
// 把整条消息掩码成「网络异常」）。英文根因刻意译成中文，同样是为了
// 避开前端的 isNetworkFailure 关键词掩码。
func describeTransportError(err error) string {
	...
	endpoint := ""
	if m := socksProxyEndpointPattern.FindStringSubmatch(cause); m != nil {
		endpoint = m[1]
	} else if m := httpProxyEndpointPattern.FindStringSubmatch(cause); m != nil {
		endpoint = m[1]
	} else if m := directDialEndpointPattern.FindStringSubmatch(cause); m != nil {
		endpoint = m[1]
	} else if m := directTargetEndpointPattern.FindStringSubmatch(cause); m != nil {
		endpoint = m[1]
	}
	reason := cause
	if idx := strings.LastIndex(cause, ": "); idx >= 0 {
		reason = strings.TrimSpace(cause[idx+2:])
	}
	reason = strings.TrimPrefix(reason, "unknown error ")
	reason = translateTransportReason(reason)
	if endpoint != "" {
		return fmt.Sprintf("出站连接失败（%s %s），任务未能提交到上游", endpoint, reason)
	}
	return fmt.Sprintf("出站连接失败（%s），任务未能提交到上游", reason)
}

func translateTransportReason(reason string) string {
	lower := strings.ToLower(reason)
	switch {
	case strings.Contains(lower, "connection refused"):
		return "连接被拒，代理可能已停止运行"
	...
	case strings.Contains(lower, "eof"):
		return "连接被中断"
	case strings.Contains(lower, "proxy protocol error"):
		return "代理协议错误"
	case strings.Contains(lower, "unauthorized") || strings.Contains(lower, "authentication"):
		return "代理认证失败"
	case strings.Contains(lower, "deadline exceeded") || strings.Contains(lower, "canceled") || strings.Contains(lower, "cancelled"):
		return "等待上游响应超时"
	default:
...
// weeklyQuotaRecoveryAt 从回复文本解析上游声明的恢复时间（"预计9月28日 17:00 恢复"）。
// 未声明或解析失败返回零值。
func weeklyQuotaRecoveryAt(msg string) (time.Time, bool) {
	m := weeklyQuotaRecovery.FindStringSubmatch(msg)
	if m == nil {
	...
	now := time.Now()
	until := time.Date(now.Year(), time.Month(nums[0]), nums[1], nums[2], nums[3], 0, 0, now.Location())
	if !until.After(now) {
		// 跨年恢复（如 12 月底报错、1 月恢复）
		until = until.AddDate(1, 0, 0)
	}
	return until, true
}

// enrichQuotaCooldown 周期性额度耗尽时把冷却延长到上游声明的恢复时间
//（未声明则冷却 24 小时）；其余分类原样返回。
func enrichQuotaCooldown(ce *ClassifyError, replyText string) *ClassifyError {
	if ce == nil || ce.Kind != FailKindQuotaExhausted || !weeklyQuotaPatterns.MatchString(replyText) {
		return ce
	}
	until, ok := weeklyQuotaRecoveryAt(replyText)
	if !ok {
...
func GenerateImageOnce(ctx context.Context, cookieHeader, prompt, model, ratio, style string) ([]string, string, error) {
	...
	if block := detectBlock(events, raw, CollectText(events)); block != nil {
		return nil, "", block
	}
...
type VideoResult struct {
	...
	// Trace 会话轨迹（提交→受理→轮询→出片），诊断「受理但不出片」用。
	Trace *VideoTrace `json:"-"`
...
func mapAbilityModel(model string) string {
	...
	// 1.0 判定必须先于 fast/mini：dola-seedance-video-1.0 不含关键词但也不能误判。
...
func videoPayload(prompt, model string, duration int, ratio, localConversationID, conversationID string) map[string]any {
	ratio = normalizeDoubaoImageRatio(ratio)
