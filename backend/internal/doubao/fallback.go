...
// fallbackRetryAttempts 母片通道整体重试次数。实测（2026-09-21）同一批任务里
// 该通道时成时败：豆包侧风控/网络抖动会让会话页拉取或 vid 提取单次失败，
// 而失败即永久回退到水印转码，用户拿到的就是带「豆包AI生成」角标的成片。
const fallbackRetryAttempts = 3

func fallbackRetryDelay(attempt int) time.Duration {
	return time.Duration(attempt+1) * 4 * time.Second
}
...
func applyFallbackNoWatermark(ctx context.Context, cookieHeader string, result *VideoResult) bool {
	...
	// vid 提取依赖 URL 正则或读视频文件头 2MB，两者都可能被单次网络抖动/风控打断，重试。
	vid := ""
	for attempt := 0; attempt < fallbackRetryAttempts && vid == ""; attempt++ {
		if attempt > 0 {
			select {
			case <-time.After(fallbackRetryDelay(attempt)):
			case <-ctx.Done():
				return false
			}
		}
	...
	// 会话页拉取同样可能被风控打断（非 200 时 fetchConversationPage 返回空且不报错），重试。
	var pageHTML string
	var err error
	for attempt := 0; attempt < fallbackRetryAttempts; attempt++ {
		if attempt > 0 {
			select {
			case <-time.After(fallbackRetryDelay(attempt)):
			case <-ctx.Done():
	...
	apis := ExtractFallbackAPIs(pageHTML)
	if vid == "" {
		// vid 始终提取不到时：每个任务使用全新会话，会话页里只出现一个 fallback_api
		// 即本次成片，无需 vid 也能定位；多个则无法区分，只能放弃。
		if len(apis) == 1 {
			for v := range apis {
				vid = v
			}
			log.Printf("[doubao] fallback_api 地址提取不到 vid，命中单视频会话兜底 vid=%s", vid)
		} else {
			log.Printf("[doubao] fallback_api 通道放弃：所有地址均提取不到 vid，会话页含 %d 个视频", len(apis))
			return false
		}
	}
	fallbackAPI, ok := apis[vid]
	if !ok {
		log.Printf("[doubao] fallback_api 母片提取失败 vid=%s: 会话页中未找到该视频的 fallback_api", vid)
		return false
	}
	raw, err := fetchFallbackResponse(ctx, fallbackAPI)
	if err != nil {
	...
	result.URLs = append([]string{info.URL}, result.URLs...)
